use super::super::ctx::{Constructable, DukContext, Idx};
...
impl<'a> ToDuktape for Object<'a> {
    fn to_context(self, _ctx: &DukContext) -> Result<()> {
        self.refer.push();
        Ok(())
    }
}
...
impl<'a> FromDuktape<'a> for Object<'a> {
    fn from_context(ctx: &'a DukContext, index: Idx) -> Result<Self> {
...
impl<'a> Constructable<'a> for Object<'a> {
    fn construct(duk: &'a DukContext) -> Result<Self> {
        duk.push_object();
...
impl<'a> ArgumentList for Object<'a> {
    ...
    fn push_args(self, ctx: &DukContext) -> Result<()> {
        self.to_context(ctx)
    }
}
...
pub mod tests {
    ...
    use super::super::super::ctx::DukContext;
