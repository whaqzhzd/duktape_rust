...
pub trait FromDuktape<'de>: Sized {
    fn from_context(ctx: &'de DukContext, index: Idx) -> Result<Self>;
}
...
impl<'de> FromDuktape<'de> for () {
    fn from_context(_ctx: &'de DukContext, _index: i32) -> Result<Self> {
        Ok(())
    }
}
...
impl<'de> FromDuktape<'de> for Value {
    fn from_context(ctx: &'de DukContext, idx: Idx) -> Result<Self> {
