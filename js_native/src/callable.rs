...
pub trait Callable {
    ...
    fn call(&self, ctx: &DukContext) -> Result<i32>;
}
...
pub(crate) unsafe fn push_callable(context: &DukContext, callable: Box<dyn Callable>) {
...
impl<T: Fn(&DukContext) -> Result<i32>> Callable for (i32, T) {
    fn argc(&self) -> i32 {
        self.0
    }
...
impl<T: Fn(&DukContext) -> Result<i32>> Callable for T {
    fn argc(&self) -> i32 {
        0
    }
...
impl<T: 'static + Fn(&DukContext) -> Result<i32>> ToDuktape for T {
...
impl<T: 'static + Fn(&DukContext) -> Result<i32>> ToDuktape for (i32, T) {
    fn to_context(self, ctx: &DukContext) -> Result<()> {
        let boxed: Box<dyn Callable> = Box::new(self);
        unsafe { push_callable(ctx, boxed) };
        Ok(())
